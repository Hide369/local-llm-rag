...
DOCS_RELEVANT = (
    ...
    # csharp-spec（言語仕様、2026-09-13追加）。csharp は MS Learn の解説で、
    # 規範的な文法と意味論はこちらにしか無い。解説側に無い語を尋ねる。
    #
    # この資料だけ日本語である。検索クエリは英語へ訳されるため、BM25 は
    # コード識別子でしか当たらない。ベクトルとリランカーは多言語なので効く。
    "C#の確定代入の規則を教えてください",
    "C#のオーバーロード解決で最適な関数メンバーを選ぶ規則を教えてください",
    # python-spec（言語リファレンス、2026-09-13追加）。コーパスに Python の資料は
    # 他に無い（langchain などの例が Python なだけである）。文法と実行モデルを
    # 尋ねる形にする。
    #
    # 「Pythonのデスクリプタで__get__が呼ばれる条件」はここに置けない。実測
    # 2026-09-13: `Python descriptor __get__ called conditions` と訳され、床1.0で
    # 根拠が**0件**になる（リランカー1位が0.10）。ベクトルもBM25も
    # `3.3.2.2 Implementing Descriptors` / `3.3.2.3 Invoking Descriptors` を1位で
    # 当てており、落としているのはリランカーだけである。同じ節を狙う
    # `Python descriptor protocol __get__` なら4.48、下の言い回しなら5.10になる。
    #
    # **これは質問集合の都合ではなく、実利用で起きる。** 日本語のほうは自然な
    # 尋ね方であり、利用者がそう尋ねれば0件になる。直すなら
    # ingest/query_translation.py の指示（節の名前に寄せて訳させる）側であって、
    # 床でもコーパスでもない。
    "Pythonのデスクリプタプロトコルについて教えてください",
    "Pythonのwith文で例外が起きたときの動作を教えてください",
...
def _hit_key(hit):
    ...
    Hit はチャンクIDを持たないため、本文を足す。重複チャンクを畳んだ現在、
    この鍵は1回の検索結果の中では一意である。`chunks.id = sha256(text)` が
    主キーになり本文とチャンクIDが1対1になったこと、ingest/retrieval.py の
    search() が chunk_id ごとに Hit を1つしか作らないことによる
    （実DBで実測: chunks 538行、DISTINCT text も538）。

    畳み込み前はそうではなかった。2026-09-02・795チャンク時点のコーパスは本文が
    完全一致するチャンクを166件（20.9%、34グループ、最大12重）含み、うち同じ
    citation を持つ組もあった（「生成AI活用セミナー.pptx スライド11」が2件）。
    畳み込み直前の608出現でも21グループ・91行（15.0%、最大7重）が残っていた。
    衝突の名残の扱いは _rrf_ranks() を参照。
    """
...
def _rrf_ranks(before):
    ...
    畳み込み前は、同じ鍵に複数のチャンクがぶら下がった。辞書に順位をひとつだけ
    持たせると後勝ちで潰れ、2件が「(RRF 5位)」と表示されて4位が消えた。
    順位が抜けて見えるのは、リランカーが順位を落としたようにも読めて誤解を招く。
    そのため順位を待ち行列にし、取り出すたびに消費する形にした。

    重複を畳んだ現在、この衝突は起きない。1回の検索結果の中で _hit_key() は
    一意であり（理由はそちらの docstring）、どの待ち行列も要素1つで終わる。
    つまり待ち行列はもう働いていない機構である。

    それでも残すのは、鍵が一意である保証が _hit_key() の外の不変条件
    （chunks.id = sha256(text)）に依存しているからである。畳み込みを外す、
    本文を鍵から落とす、Hit を chunk_id ごとに1つ作る規則が崩れる——
    どれが起きても衝突は戻ってくる。dict に潰すと戻ってきたことに気づけないが、
    待ち行列なら順位が正しく配られたまま動き続ける。消すなら _hit_key() の
    一意性を独立に守るテストを先に置くこと。
    """
