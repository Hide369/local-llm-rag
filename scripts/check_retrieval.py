...
DOCS_RELEVANT = (
    ...
    # python-spec（言語リファレンス、2026-09-13追加）。Python の資料はこれ1本で、
    # 解説側の資料は持っていない。規範的な規則を尋ねる形にする。
    #
    # 節の名前から離れた言い回しほど外れるのは go-spec と同じである。実測
    # 2026-09-13: `for` 文を尋ねると訳が `Python for loop iterable handling iter
    # __iter__ next` になり、床1.0で残る1件が `8.9.2. The async for statement`
    # になる。`8.3. The for statement` は候補にも入らない。`for` という語が
    # 非同期版の節と競合するためで、取り込みの問題ではない（4節）。
    "Pythonの名前解決とスコープの規則を教えてください",
    "Pythonのデスクリプタはどのように属性アクセスへ介入しますか",
    # typescript（公式ドキュメントの統合版、2026-09-16追加）。Docs と TSConfig
    # Reference の2つを含むので、型システムと設定ファイルの両方から1問ずつ置く。
    "TypeScriptでユニオン型を絞り込む書き方を教えてください",
    "TypeScriptのtsconfig.jsonでstrictを有効にすると何が変わりますか",
