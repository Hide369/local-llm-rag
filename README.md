...
`chroma_db/` 内に共存している。次元が異なるベクトルを混在させないよう、意図的に別
コレクションにしている（`ingest/store.py` 参照）。現在 `local_docs_v2` は `source/` 直下8ファイルと
...
- **`scripts.ingest_source` の実行中は `chroma_db` を開く他のプロセスを一切起動しない
  こと。** `chroma_db/` は複数のコレクションを1つのストアに格納しており、コレクションが
  別でも書き込み先のファイルは同一である。ChromaDBの
  `PersistentClient` は複数プロセスからの同時書き込み・読み込みを想定しておらず、
  実際に別のStreamlitプロセスが `chroma_db` を開いたまま
  ...
  削除＋全量再取り込み）は共通である。取り込み前に `Get-Process` 等でStreamlitが
  起動していないことを必ず確認すること。
