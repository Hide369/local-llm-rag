...
回答生成のモデルはサイドバーのプルダウンで切り替えられる。選べるのは下の表で
実測した2モデルだけで、既定は `qwen2.5:7b-instruct`。別のモデルを試すときは
`rag_chat_app.py` の `MODELS` に追記する（`ollama pull` 済みであること）。
同じ経路（条件抽出→絞り込み／検索→生成）で4回ずつ実測した（2026-08-13）。
