...
現行アプリには利用者別の認証・資料ACLがなく、**認証された全利用者が同じ資料を検索でき、サイドバーの「差分を取り込む」も実行できる**。本手順だけで検索専用ユーザーと管理者を分離できるわけではない。取り込み権限を与えない利用者へ公開する前にはUIの削除・権限制御などの実装変更が必要。SQLiteとメモリー内キャッシュを使うため、複数レプリカではなく1プロセスから運用する。
