...
> **導入は見送った（2026-09-16）。**
> このリポジトリは Ollama（`gpt-oss:20b` / `bge-m3` / `qwen2.5vl:7b`）のまま
> 運用する。以下は調査の記録として残してある。**ここで挙げたモデルを実機で
> 動かしてはいない。** 日本語の生成品質を現行の `gpt-oss:20b` と比べた実測も無い。
> 再開するときは[出典](#出典)から現物を引き直すこと。
