...
クライアント側に要るのは `config.toml` の1行と `~/.codex/AGENTS.md` への数行だけ
である。Python も Ollama も資料も要らない。埋め込み計算もリランクもサーバ機で
行う。
...
### 検索を呼ばせる指示は toml では書けない

toml が教えるのは接続先だけである。**どの質問で検索するか**はエージェントの判断に
委ねられ、それは別の場所で決まる。ツールの説明文は「社内資料を検索し、根拠となる
原文を出典つきで返す」だけで
（[rag_mcp_server.py:205](../scripts/rag_mcp_server.py#L205)）、呼ぶべき条件を
含まない。エージェントが自分の知識で答えてしまえば、サーバが結果に添える歯止め
（`_FALLBACK` / `_CAUTION`）は一度も読まれずに終わる。

そこで各開発者の `~/.codex/AGENTS.md` に、本リポジトリの [AGENTS.md](../AGENTS.md)
から「社内資料の参照」節をそのまま貼る。ここに置いた AGENTS.md は Codex が
リポジトリを問わず読むので、開発者が別のリポジトリで作業していても効く（場所は
`CODEX_HOME` の直下である。既定が `~/.codex` であり、`codex doctor` の
`CODEX_HOME` 欄で確認できる）。

**冒頭の開発ガイドライン部分は貼らないこと。** superpowers スキル・PowerShell・
仮想環境の取り決めであり、資料を引くだけの利用者には無関係である。

context7 も併用するなら「外部ドキュメントの参照」節も貼る。

### AGENTS.md が無いと何が壊れるか

0件時・無関係時の出口（一般知識でなら答えてよい、社内固有の事項は推測しない）は
サーバが検索結果に毎回添えるので、AGENTS.md が無くても効く。二重になっているのは
意図的である。片方だけに置くと、置いていない側の経路で歯止めが消える。

AGENTS.md にしか無いのは次の2つで、どちらもサーバからは手が届かない。

- そもそも `search_documents` を呼ぶ、という判断。呼ばれなければサーバの文言は
  存在しないのと同じである
- 社内固有の情報を context7 のクエリ文に含めない、という制約。context7 は社外の
  サービスであり、サーバを経由しないため、ここを落とすと社内の語がそのまま外へ出る

## 動作確認

Codex に「社内資料で〜を調べて」と頼み、出典つきの原文が返ることを見る。

接続はできているのに `search_documents` が一度も呼ばれない場合は
[mcp-tool-not-called.md](mcp-tool-not-called.md) にある。サーバのアクセスログでは
握手と検索を区別できないため、到達しているように見えて一度も検索していない状態が
成立する。
...
資料と無縁の質問（ライブラリの使い方など）も投げてみること。**0件のときは検索を
打ち切り、gpt-oss が自分の知識で答える**のが正しい挙動である。そのとき回答には
「社内資料に基づかない一般的な回答である」旨が添えられる。

ウェブ検索を何度も繰り返した末に「回答できません」で止まるなら、サーバ側の
`_FALLBACK`（[rag_mcp_server.py:37-43](../scripts/rag_mcp_server.py#L37-L43)）が
古い。初版は「推測で答えず、社内資料からは回答できない旨を伝えてください」と
書いており、自分の知識を封じられたエージェントが代替の根拠を外へ探しにいって
いた。現行の文言は探索を打ち切ってよいことまで明示している。

一方で**社内固有の事項（規程・議事録・社内の決定）は、0件でも推測させない**。
「就業規則の有給日数」のような質問を検索が外したときに、それらしい日数が創作
されないことを確かめておくこと。この線引きはサーバの返す文言と
[AGENTS.md](../AGENTS.md) の両方が受け持つ。
