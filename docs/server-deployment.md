...
> Windowsサーバーへ配置する場合は [Windowsサーバー版の手順書](server-deployment-windows.md) を使用する。このページはLinux/systemd向けの資料として残す。
...
|資料の取り込み|画面のサイドバーから2通り実行できる。`source/`の差分取り込みと、利用者のブラウザーからのアップロード（「資料をアップロード」ダイアログ）|利用者へ公開するなら、両方の取り込みUIを削除・管理者専用にする実装変更が必要。|
|スケールアウト|SQLiteとプロセス内キャッシュを使う|複数レプリカではなく、まず1プロセスで運用する。|

特に、現在の画面にある「差分を取り込む」と「資料をアップロード」は認証済みの全利用者に見える。資料の書き込み権限を利用者へ渡さない方針なら、サーバー公開前にこれらの操作を削除するか、管理者だけが使える別の運用経路へ分ける。

アップロードは`source/`を触れない利用者が資料を持ち込むための経路である。原本はサーバーに残らないが、**取り込んだチャンクはベクトルDBに入り、全利用者の検索対象になる**。DBもStreamlitプロセスも1つで、利用者ごとの分離はない。持ち込んだ本人が消すまで残るので、機密資料の扱いを運用規則で定めること。アップロードした資料はダイアログ内の一覧から削除できる（`uploads/`で始まる資料キーが該当する）。
