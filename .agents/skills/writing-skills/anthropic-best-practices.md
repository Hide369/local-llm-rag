...
* **クロード俳句** (高速、経済的): スキルは十分な指導を提供しますか?
* **クロード ソネット** (バランス): スキルは明確で効率的ですか?
...
※「APIエンドポイント」、「URL」、「APIルート」、「パス」を混在させてください
※「フィールド」、「ボックス」、「エレメント」、「コントロール」を混在させる
*「抽出」、「プル」、「取得」、「取得」を混在させる
...
仮定ではなくこれらの観察に基づいて繰り返します。スキルのメタデータ内の「名前」と「説明」は特に重要です。エージェントは、現在のタスクに応じてスキルをトリガーするかどうかを決定するときにこれらを使用します。スキルが何を行うのか、いつ使用する必要があるのか​​を明確に説明していることを確認してください。
