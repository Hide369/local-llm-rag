...
source_path: docs/csharp/advanced-topics/performance/interpolated-string-handler.md
...
You can write a custom [*interpolated string handler*](#implement-the-handler-pattern). An interpolated string handler is a type that processes the placeholder expression in an interpolated string. Without a custom handler, the system processes placeholders similar to <xref:System.String.Format*?displayProperty=nameWithType>. Each placeholder is formatted as text, and then the components are concatenated to form the resulting string.
