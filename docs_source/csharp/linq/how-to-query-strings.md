...
source_path: docs/csharp/linq/how-to-query-strings.md
...
The following example shows how to use a LINQ query to count the occurrences of a specified word in a string. To perform the count, first the <xref:System.String.Split*> method is called to create an array of words. There's a performance cost to the <xref:System.String.Split*> method. If the only operation on the string is to count the words, consider using the <xref:System.Text.RegularExpressions.Regex.Matches*> or <xref:System.String.IndexOf*> methods instead.
...
The call to <xref:System.String.Split*> uses punctuation marks as separators in order to remove them from the string. If you didn't do remove punctuation, for example you could have a string "Historically," that wouldn't match "Historically" in the `wordsToMatch` array. You might have to use extra separators, depending on the types of punctuation found in the source text.
