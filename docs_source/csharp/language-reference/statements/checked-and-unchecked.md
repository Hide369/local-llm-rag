...
source_path: docs/csharp/language-reference/statements/checked-and-unchecked.md
...
> [!NOTE]
> The overflow behavior of *user-defined* operators and conversions can differ from the behavior described in the preceding paragraph. In particular, [user-defined checked operators](../operators/arithmetic-operators.md#user-defined-checked-operators) might not throw an exception in a checked context.
...
Constant expressions are evaluated by default in a checked context and overflow causes a compile-time error. You can explicitly specify an unchecked context for a constant expression by using the `unchecked` statement or operator.
